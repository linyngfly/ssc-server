...
class DownloadRes {
        ...
        if (figure_url == 'default.png' || figure_url == 'jiaodie.png' || figure_url.indexOf('upload/boys/') > -1 ||
            ...
            try {
                let {
                    data,
                    headers
                } = await this._getRemoteResource(ctx.request.headers, figure_url);
    ...
    _getRemoteResource(headers, url) {
        let req_headers = {};
        ...
        return new Promise(function (resolve, reject) {
            ...
            req.on('error', function (error) {
                logger.error('_getRemoteResource error:', error);
                reject(error);
            });
            req.end();
