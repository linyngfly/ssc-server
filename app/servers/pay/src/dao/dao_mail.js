...
function _prepareReadMail(data, cb) {
    ...
    if (!CommonUtil.isParamExist("dao_mail", mid, "接口调用请传参数id(阅读的邮件ID)", cb)) return false;
    ...
    logger.error('mail_box:', mail_box);
    logger.error('mid:', mid);
    if (tools.ArrayUtil.contain(mail_box, Number(mid))) {
        cb && cb(null, 'Find Mail');
        return true;
    }
    else {
        logger.error("没有邮件");
...
function _didReadMail(data, cb) {
    ...
    if (account) {
        let uid = account.id;
        // 获取邮件详细信息.
        _getMailsDetail([mid], function (err, mails) {
            ...
            // 发放奖励.
            if (mail_reward != null) {
                logger.info("mail_reward:", mail_reward);
                mail_reward = JSON.parse(mail_reward);

                let item_list = BuzzUtil.getItemList(mail_reward);
                ...
                DaoReward.getReward(account, mail_reward, function (err_get_reward, results_get_reward) {
                    if (err_get_reward) {
                    ...
                    // 缓存操作
                    CacheAccount.deleteMail(account, mid);
                    logger.error('mail:', mail);

                    recordReceivedUid(mail, uid, mid);

                    ret.change.mail_box = account.mail_box;
                    ...
                    if (mail_reward[0][0] == "i410") {
                        //统计收到鲜花
                        buzz_receive_flower.flower_receive(account.id, mail_reward, function (currentTotal, total) {
                            if (account.privacy == 1 && account.test >= 0) {
                    ...
                    for (let i = 0; i < item_list.length; i++) {
                        ...
                        let item_id = reward[0];
                        let item_num = reward[1];
                        logger.info(FUNC + "item_id:", item_id);
                        logger.info(FUNC + "item_num:", item_num);
                        gain_item_list.push({
                            item_id: item_id,
                            item_num: item_num
                        });
...
/**
 * 在tlb_mail中记录已经收到了邮件的用户
 * @param {*} mail 邮件对象
 * @param {*} uid 用户ID
 * @param {*} mid 邮件ID
 */
function recordReceivedUid(mail, uid, mid) {
    let receiverUid = mail.receiverUid || '[]';// 可以领取邮件的玩家列表
    let receivedUid = mail.receivedUid || '[]';// 已领取邮件玩家列表
    receiverUid = JSON.parse(receiverUid);
    receivedUid = JSON.parse(receivedUid);
    if (!tools.ArrayUtil.contain(receivedUid, uid)) {
        receivedUid.push(uid);
        tools.SqlUtil.query('update tbl_mail set receivedUid=? where id=? ', [JSON.stringify(receivedUid), mid]);
    }
}
...
function _prepareSendMail(data, cb) {
    ...
    try {
        ...
        if (invalid_item_list.length > 0) {
            let extraErrInfo = {
                debug_info: "不存在的物品:" + invalid_item_list.toString()
            };
