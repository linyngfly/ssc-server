...
class CQBetParser{
    ...
    checkType(betData) {
        let type = config.CQSSC.Bet_Type.UNKNOWN;
        if(betData.match(this.reg1)){
            type = config.CQSSC.Bet_Type.SIZE;
        }else if(betData.match(this.reg2)){
            type = config.CQSSC.Bet_Type.POS;
        }else if(betData.match(this.reg3)){
            type = config.CQSSC.Bet_Type.NUM;
        }else if(betData.match(this.reg4)){
            type = config.CQSSC.Bet_Type.BS1;
        }else if(betData.match(this.reg5)){
            type = config.CQSSC.Bet_Type.BS2;
        }else if(betData.match(this.reg6)){
            type = config.CQSSC.Bet_Type.BS3;
        }
        return type;
    }
    ...
    CQBetParser.prototype.handlePos = function (betData) {
        let parseResult = {
            error:Code.OK,
            total:0,
            betTypeInfo:{},
    ...
    CQBetParser.prototype.handleNum = function (betData) {
        let parseResult = {
            error:Code.OK,
            total:0,
            betTypeInfo:{},
    ...
    CQBetParser.prototype.handleBS1 = function (betData) {
        let parseResult = {
            error:Code.OK,
            total:0,
            betTypeInfo:{},
    ...
    CQBetParser.prototype.handleBS2 = function (betData) {
    ...
    CQBetParser.prototype.handleBS3 = function (betData) {
        let parseResult = {
            error:Code.OK,
            total:0,
            betTypeInfo:{},
    ...
    CQBetParser.prototype.parse = function(data, cb){

        var parseRet = null;
        switch (this.checkType(data)){
            case config.CQSSC.Bet_Type.SIZE:{
                parseRet = this.handleSize(data);
            }
                break;
            case config.CQSSC.Bet_Type.POS:{
                parseRet = this.handlePos(data);
            }
                break;
            case config.CQSSC.Bet_Type.NUM:{
                parseRet = this.handleNum(data);
            }
                break;
            case config.CQSSC.Bet_Type.BS1:{
                parseRet = this.handleBS1(data);
            }
                break;
            case config.CQSSC.Bet_Type.BS2:{
                parseRet = this.handleBS2(data);
            }
                break;
            case config.CQSSC.Bet_Type.BS3:{
