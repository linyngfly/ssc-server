...
class CQBetParser{
    ...
    _checkType(betData) {
        ...
        let splitData = null;
        for(;;){
            splitData = betData.match(this._reg1);
            if(splitData){
    ...
    _handlePos(splitData) {
        ...
        let ballPos = splitData[1].match(this._splitReg);
        ...
        let perMoney = parseInt(splitData[3],10);
        let err = this._checkMoney(perMoney);
        if (err) {
            return [err];
        }
    ...
    _handleNum(splitData) {
        ...
        for (let i = 0; i< types.length;++i){
            for (let j=1;j<=5;j++){
                ...
                tempItem.desc = `${tempItem.result}${config.DES_SEPARATOR}${perMoney}`;
    ...
    _handleBS2(splitData) {
        ...
        let perMoneys = [];
        perMoneys.push(Number(splitData[2]));
        perMoneys.push(Number(splitData[3]));
        perMoneys.push(Number(splitData[4]));

        let sum =  perMoneys.reduce(function (previous, current) {
            return previous + current;
        });

        let err = this._checkMoney(sum);
        if (err) {
            return [err];
        }
        ...
        for (let j=0;j<this._keyValue.length;j++){
            for (let i = 0; i< types.length;++i){
                ...
                tempItem.desc = `${tempItem.result}${config.DES_SEPARATOR}${tempItem.money}`;
