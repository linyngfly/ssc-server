...
(function() {
  ...
  pomelo.init = function(params, cb) {
    ...
    var url = 'ws://' + host;
    if (window.SDKConfig && SDKConfig.server_head.indexOf('https:') >= 0) {
      url = 'wss://' + host;
    }
    if(port) {
      url +=  ':' + port;
  ...
  var connect = function(params, url, cb) {
    ...
    var onclose = function(event) {
      ...
      if(user_reconnect && !!params.reconnect && reconnectAttempts < maxReconnectAttempts) {
        reconnect = true;
        reconnectAttempts++;
  ...
  pomelo.disconnect = function(reconect, cb) {
    user_reconnect = reconect;
    disconnectCb = cb;
    if(socket) {
